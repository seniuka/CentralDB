...
END"
        ...
        write-log -Message "OHMaintenanceSolution | $Type | Started Creating Objects for $Type on Server $server connected to instance $instance"
        ...
        write-log -Message "OHMaintenanceSolution | $Type | Completed Creating Objects for $Type on Server $server connected to instance $instance"
...
try 
    { 	
        ...
        write-log -Message "OHMaintenanceSolution | $Type | Starting cleanup of maintenance objects on $server connected to instance $instance" -Path $LogPath
        ...
        write-log -Message "OHMaintenanceSolution | $Type | Completed cleanup of maintenance objects on $server connected to instance $instance" -Path $LogPath
    }    
catch 
	{ 
        $ex = $_.Exception 
...
try 
    { 
	   ...
	   #$BackupPath = "\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf('.')) + ".195" + "\SQLBackups\"
        ...
        write-log -Message "OHMaintenanceSolution | $Type | Started execution of $TypeName on $server connected to instance $instance" -Path $LogPath                
        ...
        {
            write-log -Message "OHMaintenanceSolution | $Type | On $inst, there are no rows in the table or table does not exist."  -Path $LogPath
        }
    }    
catch 
	{ 
...
#Execute Script
 try
 {
     ...
     $LogPath = $LogPath  + "OHMaintenanceSolution.log"

     if($Type.Length -ne 0)
     {        
        $LogPath = $origBasePath + $env:computername + "_OHMaintenanceSolution_" + $Type + ".log"
        ##$InstanceNameanceObjectList = [System.Data.Sql.SqlDataSourceEnumerator]::Instance.GetDataSources() | ? {$_.ServerName -eq $env:computername }
        $InstanceNameanceObjectList = Get-WmiObject win32_Service -filter "state = 'running'" | where {$_.Name -match "MSSQL\$"} | SELECT @{Expression={$env:computername};Label="Server"}, @{Expression={$(($_.name).ToString().Substring(6))};Label="Instance"}	  
        $mSOL = $InstanceNameanceObjectList | measure

        if ($mSOL.Count -gt 0)
        {
            foreach ($element in $InstanceNameanceObjectList)
            {    
		    ...
		    {
                $message = "$Type | could not find server While executing maintenance solution"
                write-log -Message $message -Path $LogPath            
                Throw $message
		    }

		    #$LogPath = $origBasePath  +  $server + "_CommandLog_" + $Type + ".log"
		    $ElapsedTime = [System.Diagnostics.Stopwatch]::StartNew()
		
		    if ($Type -eq "CLEAR")
		    {
			    write-log -Message "OHMaintenanceSolution | $Type | Log Cleared at $(get-date)" -Path $LogPath -Clobber
		    }
		    else
		    {
			    write-log -Message "OHMaintenanceSolution | $Type | Script Started at $(get-date)" -Path $LogPath
		    }
    	    ...
    	    If ($responds) 
            {
                ...
                write-log -Message "OHMaintenanceSolution | $Type | $server Connected to instance $instance"  -Path $LogPath
    	    ...
    	    else 
            {
		        ...
		        write-log -Message "OHMaintenanceSolution | $Type | $server Server did not respond"  -Path $LogPath
       	    }
        
            write-log -Message "OHMaintenanceSolution | $Type | Total Elapsed Time: $($ElapsedTime.Elapsed.ToString())"  -Path $LogPath
            }
        }
        else
        {
            $message = "OHMaintenanceSolution | $Type | could not find an instance While executing maintenance solution"
            write-log -Message $message -Path $LogPath            
            Throw $message
        }
     }
    write-log -Message "OHMaintenanceSolution | $Type | Script Ended at $(get-date)" -Path $LogPath
