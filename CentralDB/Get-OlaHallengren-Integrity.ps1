...
param(
	...
	[string]$OlaHallengrenCommandActionFilter="DatabaseIntegrityCheck.sql", #If you rename them... what are they named by default DatabaseIntegrityCheck.sql...

	[string]$logPath="",
	[string]$logFileName="Get-OlaHallengren-Integrity_" + $env:computername + ".log"
	)
...
function Get-OlaHallengren($svr, $inst, $type) 
{
	...
	{	
        ...
        {	
            ...
            $sc = $InstanceSQLConn.CreateCommand()                    
            Write-Log -Message "### EXECUTING DatabaseIntegrityCheck ############################################" -Level Info -Path $logPath			 
            ...
            if (![string]::IsNullOrEmpty($MaxDOP)){$query = $query + "@MaxDOP= $MaxDOP,"}
            ...
            Write-Log -Message "### CENTRALIZING DatabaseIntegrityCheck Data ############################################" -Level Info -Path $logPath	
            ...
            Write-Log -Message "### CLEAR LOCAL DatabaseIntegrityCheck Data #########################################" -Level Info -Path $logPath
