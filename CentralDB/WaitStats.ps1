...
#	[string]$errorPath="\\p-sec-mfs006\SQL_Backup\_CentralDB\ErrorLog\WaitStats_" + $env:computername + ".log"
    [string]$errorPath="\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf('.')) + ".195" + "\SQL_NASBackup\_CentralDB\Errorlog\WaitStats_" + $env:computername + ".log"
	)
...
$query= ";WITH [Waits] AS
 ...
 SELECT ('$Svr') as ServerName, ('$inst') as InstanceName, 
      ...
      INNER JOIN [Waits] AS [W2]
         ON [W2].[RowNum] <= [W1].[RowNum] and [W2].[wait_type] = [W1].[wait_type]
