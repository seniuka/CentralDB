...
param(
    ...
    [int32]$CommandTimeout=10800,
##    [string]$BasePath      ="\\p-sec-mfs006\SQL_Backup\_CentralDB\Errorlog\",
    [string]$BasePath      ="\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf('.')) + ".195" + "\SQL_NASBackup\_CentralDB\Errorlog\",
##    [string]$BackupPath    ="\\p-sec-mfs006\SQL_Backup\"
	[string]$BackupPath    ="\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf('.')) + ".195" + "\SQL_NASBackup\"   
...
{ 
   ...
   try 
    { 
        ...
        #$bulkCopy
        #write-log -Message "connectionString" -Path $BasePath
        $bulkCopy.DestinationTableName = $tableName 
        #write-log -Message "TableName $tableName" -Path $BasePath
        $bulkCopy.BatchSize = $BatchSize 
        $bulkCopy.BulkCopyTimeout = $QueryTimeOut 
        $bulkCopy.WriteToServer($Data) 
        #write-log -Message "write to server" -Path $BasePath
        $conn.Close() 
    } 
    Catch [System.Management.Automation.MethodInvocationException]
    {
	$ex = $_.Exception 
	write-log -Message "$ex.Message on $svr" -Level Error  -Path $BasePath
    }
    catch 
    { 
        $ex = $_.Exception 
...
######################################################################################################################################
#Fucntion to BuildMaintenancePlan
function BuildMaintenancePlan($svr, $inst, $type, $GUID) 
{
...
try 
    { 	
        ...
        $IndexOptimize = "IndexOptimize_" + $GUID
        $DatabaseIntegrityCheck = "DatabaseIntegrityCheck_" + $GUID
        $DatabaseBackup = "DatabaseBackup_" + $GUID
...
IF OBJECT_ID('[dbo].[$TableName]') IS NULL AND OBJECT_ID('[dbo].[PK_$TableName]') IS NULL
BEGIN
CREATE TABLE [dbo].[$TableName](
[ID] int IDENTITY(1,1) NOT NULL CONSTRAINT [PK_$TableName] PRIMARY KEY CLUSTERED,
...
BEGIN
  ...
  IF @LogToTable = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'dbo' AND objects.[name] = '$TableName')
  BEGIN
  ...
  IF @LogToTable = 'Y'
  BEGIN
    INSERT INTO dbo.[$TableName] (DatabaseName, SchemaName, ObjectName, ObjectType, IndexName, IndexType, StatisticsName, PartitionNumber, ExtendedInfo, CommandType, Command, StartTime)
...
@LogToTable nvarchar(max) = 'N',
@WithTABLERESULTS nvarchar(max) = 'N',
@Execute nvarchar(max) = 'Y'
...
BEGIN
  ...
  SET @StartMessage = @StartMessage + ', @LogToTable = ' + ISNULL('''' + REPLACE(@LogToTable,'''','''''') + '''','NULL')
  SET @StartMessage = @StartMessage + ', @Execute = ' + ISNULL('''' + REPLACE(@Execute,'''','''''') + '''','NULL') + CHAR(13) + CHAR(10)
  SET @StartMessage = @StartMessage + ', @WithTABLERESULTS = ' + ISNULL('''' + REPLACE(@WithTABLERESULTS,'''','''''') + '''','NULL') 
  SET @StartMessage = @StartMessage + 'Source: https://ola.hallengren.com' + CHAR(13) + CHAR(10)
  SET @StartMessage = REPLACE(@StartMessage,'%','%%') + ' '
  RAISERROR(@StartMessage,10,1) WITH NOWAIT
  ...
  END
   ...
   IF @WithTABLERESULTS NOT IN ('Y','N') OR @WithTABLERESULTS IS NULL
  BEGIN
    SET @ErrorMessage = 'The value for the parameter @WithTABLERESULTS is not supported.' + CHAR(13) + CHAR(10) + ' '
    RAISERROR(@ErrorMessage,16,1) WITH NOWAIT
    SET @Error = @@ERROR
  END
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
		...
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand10 = cast(newID() as nvarchar(36))  
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + 'DECLARE @ID int; DECLARE @xml_output xml; SELECT @ID = MAX(ID) FROM dbo.[$TableName];'
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + 'IF (CAST(SERVERPROPERTY(''ProductVersion'') AS VARCHAR) LIKE ''10%'') BEGIN execute (''CREATE TABLE [##tmpDBCC_output_' + @CurrentCommand10 + '] ([Error] INT, [Level] int, [State] int, [MessageText] varchar(7000), [RepairLevel]  varchar(7000), [Status] int, 		[DbId] int, [ObjectID] bigint, [IndexId] bigint, [PartitionId] bigint, [AllocUnitId] bigint, [File] int, [Page] int, [Slot] int,  		[RefFile] int, [RefPage] int, [RefSlot] int, [Allocation] int	)'') END	ELSE BEGIN execute (''CREATE TABLE [##tmpDBCC_output_' + @CurrentCommand10 + '] 	([Error] int, [Level] int, [State] int, [MessageText] varchar(7000), [RepairLevel] varchar(7000), [Status] int, [DbId] int, [DbFragId] int, [ObjectID] bigint, [IndexId] bigint, [PartitionId] bigint, [AllocUnitId] bigint, [RidDbId] int, [RidPruId] int, [File] int, [Page] int, [Slot] int, [RefDbID] int, [RefPruId] int, [RefFile] int, [RefPage] int, [RefSlot] int, [Allocation] int)'') END;'
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + 'INSERT INTO [##tmpDBCC_output_' + @CurrentCommand10	+ ']'	
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + 'EXECUTE('''
		SET @CurrentCommand01 = @CurrentCommand01 + 'DBCC CHECKDB (' + QUOTENAME(@CurrentDatabaseName)
        IF @NoIndex = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + ', NOINDEX'
        SET @CurrentCommand01 = @CurrentCommand01 + ') WITH /*NO_INFOMSGS,*/ ALL_ERRORMSGS'
        IF @PhysicalOnly = 'N' SET @CurrentCommand01 = @CurrentCommand01 + ', DATA_PURITY'
        IF @PhysicalOnly = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + ', PHYSICAL_ONLY'
        IF @ExtendedLogicalChecks = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + ', EXTENDED_LOGICAL_CHECKS'
        IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + ', TABLERESULTS'
		IF @TabLock = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + ', TABLOCK'
		IF @MaxDOP IS NOT NULL SET @CurrentCommand01 = @CurrentCommand01 + ', MAXDOP = ' + CAST(@MaxDOP AS nvarchar)
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + ''')'
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + '; SELECT @xml_output = xml_output FROM (SELECT * FROM [##tmpDBCC_output_' + @CurrentCommand10 + '] as CHECKDB FOR XML AUTO, TYPE, ROOT(''DBCC'')) x(xml_output) '
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + '; UPDATE dbo.[$TableName] SET ExtendedInfo = @xml_output WHERE ID = @ID'
		IF @WithTABLERESULTS = 'Y' SET @CurrentCommand01 = @CurrentCommand01 + '; IF EXISTS (SELECT ''x'' FROM sys.objects WHERE object_id = OBJECT_ID(N''##tmpDBCC_output_' + @CurrentCommand10 + ''') AND type IN ( N''U'')) BEGIN DROP TABLE [##tmpDBCC_output_' + @CurrentCommand10 + '] END'
...
END"
        #Query1 = Setup and Table 	
        #Query2 = CommandExec Proc
        #Query3 = Backup Proc
        #Query4 = DBCC Proc
        #Query5 = INDEX Proc

        write-log -Message "$Type | Started Creating Objects for $Type on Server $server connected to instance $instance"

        $da = new-object System.Data.SqlClient.SqlDataAdapter ($query1, $cn)
        $dt = new-object System.Data.DataTable
        $da.fill($dt) | out-null

        $da = new-object System.Data.SqlClient.SqlDataAdapter ($query2, $cn)
        $dt = new-object System.Data.DataTable
        $da.fill($dt) | out-null
        ...
        switch($Type)
        {
            "DBCC" 
            {
                $da = new-object System.Data.SqlClient.SqlDataAdapter ($query4, $cn)
                $dt = new-object System.Data.DataTable
                $da.fill($dt) | out-null
            }
            "IDXF" 
            {   
                $da = new-object System.Data.SqlClient.SqlDataAdapter ($query5, $cn)
                $dt = new-object System.Data.DataTable
                $da.fill($dt) | out-null
            }
            "IDXS" 
        ...
        write-log -Message "$Type | Completed Creating Objects for $Type on Server $server connected to instance $instance"
	...
	{ 
        $ex = $_.Exception 
	    write-log -Message "$Type | $ex.Message on $server While creating objects for maintenance solution" -Path $BasePath
	} 
finally{
...
try 
    { 	
        ...
        switch($Type)
        {
            "DBCC" 
            {                        
                $query2 = "IF OBJECT_ID('[dbo].[$DatabaseIntegrityCheck]') IS NOT NULL DROP PROCEDURE [dbo].[$DatabaseIntegrityCheck];"       
            }
            "IDXF" 
            {   
                $query2 = "IF OBJECT_ID('[dbo].[$IndexOptimize]') IS NOT NULL DROP PROCEDURE [dbo].[$IndexOptimize];"
            }
            "IDXS" 
            {
                $query2 = "IF OBJECT_ID('[dbo].[$IndexOptimize]') IS NOT NULL DROP PROCEDURE [dbo].[$IndexOptimize];"
        ...
        write-log -Message "$Type | Starting cleanup of maintenance objects on $server connected to instance $instance" -Path $BasePath

        #Query1 = CLEAN UP CommandLog/Exec	
        $da = new-object System.Data.SqlClient.SqlDataAdapter ($query1, $cn)
        $dt = new-object System.Data.DataTable
        $da.fill($dt) | out-null

        #Query2 = CLEAN UP Specific Objects
        $da = new-object System.Data.SqlClient.SqlDataAdapter ($query2, $cn)
        $dt = new-object System.Data.DataTable
        $da.fill($dt) | out-null

        write-log -Message "$Type | Completed cleanup of maintenance objects on $server connected to instance $instance" -Path $BasePath
    }    
catch 
	{ 
        $ex = $_.Exception 
	    write-log -Message "$Type | $ex.Message on $server While cleaning up maintenance solution"  -Path $BasePath
	} 
finally{
   		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	}
...
try 
    { 
	    ...
	    #$BackupPath = "\\" + ((ipconfig | select-string -notmatch "169.254" | select-string -notmatch "255.255" | select-string -notmatch "192.168" | findstr [0-9].\.)[0].Split()[-1]).ToString().Remove(((ipconfig | findstr [0-9].\.)[0].Split()[-1]).ToString().LastIndexOf('.')) + ".195" + "\SQL_NASBackup\"
        ...
        switch($Type)
        {
            ...
            {
                $Command = "USE [TempDB];
				IF EXISTS ( SELECT 'x' FROM sys.objects WHERE object_id = OBJECT_ID(N'$IndexOptimize') AND type IN ( N'P', N'PC' ) )
            ...
            "TLOG" 
            {
				...
				@MaxTransferSize = 2097152,
				@LogToTable = 'Y'"
                $TypeName = "'Database Backup - Transaction Log'"
            }
        }
        
        $query= "SET NOCOUNT ON		
		$Command 		
        DECLARE @LoadGUID UNIQUEIDENTIFIER = NEWID()
        IF EXISTS (SELECT 1 FROM [TempDB].INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' and TABLE_NAME = '$TableName')
        BEGIN        
            SELECT 
			     [DatabaseName]
			    ,[SchemaName]
			    ...
			    ,[StatisticsName]
			    ,[PartitionNumber]
			    ,extendedinfo.value('(/DBCC/CHECKDB[@Error=""8989""]/@MessageText)[1]', 'nvarchar(max)') 
			    ,[Command]
			    ,[CommandType]
			    ,[StartTime]
			    ,[EndTime]
			    ...
			    ,@LoadGUID
		    FROM 
			    [TempDB].[dbo].[$TableName]
        END"
            
        write-log -Message "$Type | Started execution of $TypeName on $server connected to instance $instance" -Path $BasePath                
        ...
        #write-log -Message "RowCount $RowCount"  -Path $BasePath
        if($dt.Rows.Count -gt "0")
        {
            write-log -Message "$Type | Completed execution of $TypeName on $server connected to instance $instance" -Path $BasePath
            #write-log -Message "Write-DataTable" -Path $BasePath
            Write-DataTable -ServerInstance $SQLInst -Database $Centraldb -TableName $CITbl -Data $dt
        }
        else
        {
            write-log -Message "$Type | On $inst, there are no rows in the table or table does not exist."  -Path $BasePath
        }
    }    
catch 
	{ 
        $ex = $_.Exception 
	    write-log -Message "$Type | $ex.Message on $server While executing maintenance solution"  -Path $BasePath
	} 
finally{
   		$ErrorActionPreference = "Continue"; #Reset the error action pref to default
	}
...
######################################################################################################################################
 ...
 $BasePath = $BasePath  + $GUID.ToString() + "_CommandLog.log"

 Write-Output "Here1"

 if($Type.Length -ne 0)
 {
     #$SQLInstanceObjectList = [System.Data.Sql.SqlDataSourceEnumerator]::Instance.GetDataSources() | ? {$_.ServerName -eq $env:computername }
	  $SQLInstanceObjectList = Get-WmiObject win32_Service -filter "state = 'running'" | where {$_.Name -match "MSSQL\$"} | SELECT @{Expression={$env:computername};Label="Server"}, @{Expression={$(($_.name).ToString().Substring(6))};Label="Instance"}	
  Write-Output "Here2"
     write-log -Message "$Type | Gathered Instance List [$env:computername]" -Path $BasePath	

     foreach ($element in $SQLInstanceObjectList)
     {    
         Write-Output "Here3"
   	    # Get ServerName and InstanceName from CentralDB
	    #$server = $element.Item(0)
	    #$instance = $element.Item(0) + "\" + $element.Item(1)
              
	    $server = $element.Server
	    $instance = $element.Server + "\" + $element.Instance  
		...
		$BasePath = $origBasePath  +  $server + "_CommandLog_" + $Type + ".log"
		$ElapsedTime = [System.Diagnostics.Stopwatch]::StartNew()
		
		if ($Type -eq "FULL")
		{
			write-log -Message "$Type | Script Started at $(get-date)" -Path $BasePath	
		}
		else
		{
			write-log -Message "$Type | Script Started at $(get-date)" -Path $BasePath
	    ...
	    # If the machine responds break out of the result loop and indicate success
		    ...
		    $responds = $true #added since pings are not allowed
    	    If ($responds) 
            {
             Write-Output "Here5"
                # Calling funtion and passing server and instance parameters
                write-log -Message "$Type | $server Connected to instance $instance"  -Path $BasePath
		        $GUID = [guid]::NewGuid().Tostring()
				BuildMaintenancePlan 	$server $instance $type $GUID
				ExecuteMaintenancePlan	$server $instance $type $GUID
                CleanupMaintenancePlan	$server $instance $type $GUID
    	    }
    	    else 
            {
 	        # Let the user know we couldn't connect to the server
		        write-log -Message "$Type | $server Server did not respond"  -Path $BasePath
       	    }
     }
 }

write-log -Message "$Type | Script Ended at $(get-date)"  -Path $BasePath
write-log -Message "$Type | Total Elapsed Time: $($ElapsedTime.Elapsed.ToString())"  -Path $BasePath
write-log -Message "----------------------------------" -Path $BasePath	