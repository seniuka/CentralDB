...
{ 
   ...
   try 
    { 
        $conn.Open()
		$bulkCopyOptions = $([Data.SqlClient.SqlBulkCopyOptions]::FireTriggers).value__
		$bulkCopy = New-Object Data.SqlClient.SqlBulkCopy($connectionString, $bulkCopyOptions)		
    ...
    Catch [System.Management.Automation.MethodInvocationException]
    {
		...
		write-log -Message "$ex.Message on $ServerInstance at $line" -Level Error -NoConsoleOut -Path $logPath
    }
    catch 
    { 
        $ex = $_.Exception 
